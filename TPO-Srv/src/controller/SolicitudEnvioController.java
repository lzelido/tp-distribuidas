...
public class SolicitudEnvioController extends UnicastRemoteObject implements ISolicitudEnvioService{
	...
	public int guardarSolicitudEnvio(SolicitudEnvioDTO solicitudDTO) throws RemoteException {
		try {
			...
			if (cliente != null){
				...
				System.out.println("Error al grabar la solicitud de env�o (No existe el cliente)");
				return 0;
			}
			...
			if (!clienteHabRecep.equals("")){
				...
				System.out.println("Error al grabar la solicitud de env�o (No existe el cliente habilitado para la recepci�n)");
				return 0;
			}
			...
			if (solicitud.isEsRetiroDomicilio()){
				...
				System.out.println("No se solicit� retiro a domicilio");
			}
			...
			if (cargasDTO != null && !cargasDTO.isEmpty()){
				...
				System.out.println("Error al grabar la solicitud de env�o (No existen cargas)");
				return 0;
			}
			
			(new SolicitudEnvioSRV()).grabarSolicitud(solicitud);
			
			System.out.println("La solicitud se grab� con �xito");
			return solicitud.getCodigo();		
		}catch (Exception e) {
			e.printStackTrace();
			System.out.println("Error al grabar la solicitud de env�o");
			return 0;
		}
	}
	...
	private Carga grabarCarga (CargaDTO cargaDTO){
		try {
			...
			carga.setFragilidad(cargaDTO.getFragilidad());
	...
	private Dimension grabarDimension (DimensionDTO dimensionDTO){
		try {
			...
			if (unidadX != null){
				...
				System.out.println("Error al grabar la dimensi�n (No existe la unidad de la coordenada X)");
				return null;
			}
			...
			if (unidadY != null){
				...
				System.out.println("Error al grabar la dimensi�n (No existe la unidad de la coordenada Y)");
				return null;
			}
			...
			if (unidadZ != null){
				...
				System.out.println("Error al grabar la dimensi�n (No existe la unidad de la coordenada Z)");
				return null;
			}
			
			(new DimensionSRV()).grabarDimension(dimension);
			
			System.out.println("La dimensi�n se grab� con �xito");
			return dimension;
		}catch (Exception e) {
			e.printStackTrace();
			System.out.println("Error al grabar la dimensi�n");
			return null;
		}
	}
}
