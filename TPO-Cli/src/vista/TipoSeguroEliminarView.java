...
public class TipoSeguroEliminarView  extends javax.swing.JFrame {
	...
	public static void main(String[] args) {
		SwingUtilities.invokeLater(new Runnable() {
			public void run() {
				TipoSeguroEliminarView inst = new TipoSeguroEliminarView();
	...
	private void initGUI() {
		try {
			...
			this.setTitle("Eliminar Tipo Seguro");
			{
			...
			}
			{
				...
				jButtonEliminar.setBounds(36, 165, 85, 28);
		...
		try{
			...
			if(response == JOptionPane.NO_OPTION){
				...
				if (response == JOptionPane.YES_OPTION){
					...
					int check = 0;
					check = TipoSeguroController.getInstance().bajaTipoSeguro(tipoSeguroAct);
					if (check == 2){
						String mensaje = "Se eliminó el Tipo de Seguro";
						...
						if (check == 1){
							...
							String mensaje = "Falla en la eliminación del Tipo de Seguro";
							JOptionPane.showMessageDialog(this, mensaje, "ERROR", JOptionPane.ERROR_MESSAGE);
						}
					}
					
					TipoSeguroEliminarView.this.dispose();
//					new Principal();
				}
			}
		}catch (Error e){
			...
			System.out.println("Mensaje Error al eliminar un Tipo de Seguro: " + e.getMessage());
			System.out.println("Stack Trace al eliminar un Tipo de Seguro: " + e.getStackTrace());
			JOptionPane.showMessageDialog(this, e, "Error", JOptionPane.ERROR_MESSAGE);
		}
	}
